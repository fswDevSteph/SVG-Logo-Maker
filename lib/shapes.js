...
class Shape {
  ...
  generateSVG() {
    return `<svg height="100" width="100" xmlns="http://www.w3.org/2000/svg">${this.render()}${this.generateText()}</svg>`;
  }
}
...
class Circle extends Shape {
  ...
  generateText() {
    return `<text x="10" y="15" fill="${
      this.textcolor
    }" font-size='4'>${this.render()}${this.text}</text>`;
  }
}
class Triangle extends Shape {
  ...
  generateText() {
    return `<text x="10" y="15" fill="${this.textcolor}" font-size='4'>${this.text}</text>`;
  }
}
